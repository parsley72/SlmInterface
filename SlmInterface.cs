...
*/
        ...
        /// int
        public int m_nSamplePeriodInMSecs;
...
//        [System.Runtime.InteropServices.MarshalAsAttribute(System.Runtime.InteropServices.UnmanagedType.ByValArray, SizeConst = 2000, ArraySubType = System.Runtime.InteropServices.UnmanagedType.U1)]
//        public byte[] m_pbyData;
        public System.IntPtr m_pbyData;
*/
        /// wchar_t[260 /*MAX_PATH*/]
        [System.Runtime.InteropServices.MarshalAsAttribute(System.Runtime.InteropServices.UnmanagedType.ByValTStr, SizeConst = 260 /*MAX_PATH*/)]
        public string uszWavFile;
    }
    ...
    public partial class NativeMethods
    {
        ...
        [return: System.Runtime.InteropServices.MarshalAsAttribute(System.Runtime.InteropServices.UnmanagedType.I1)]
        private static extern bool SI_GetBulkDataEx_DLL(int nSamplePeriodInMSecs, ref slmSBulkRecords_DLL pSlmBulkRec);

        public static bool SI_GetBulkDataEx(slmSSettings settings, ref slmSBulkRecords pSlmBulkRec)
        {
            ...
            bool bReturn = false;
            switch (bulkRecords_DLL.eRecType)
            {
                case eTypeBulk.eTypeBulkTh:
                {
                    int nLength = Marshal.SizeOf(typeof(slmSTimeHistory)) * pSlmBulkRec.m_dataCounts.m_nNumTH;
                    bulkRecords_DLL.m_pThRecs = Marshal.AllocHGlobal(nLength);
                    Marshal.Copy(new byte[nLength], 0, bulkRecords_DLL.m_pThRecs, nLength);
                    bReturn = SI_GetBulkDataEx_DLL(settings.m_nSamplePeriodInMSecs, ref bulkRecords_DLL);
                    if (bReturn)
                    {
                ...
                case eTypeBulk.eTypeBulkEvnt:
                {
                }
                break;

                case eTypeBulk.eTypeBulkMeas:
                {
                }
                break;

                case eTypeBulk.eTypeBulkRunRec:
                {
                }
                break;

                case eTypeBulk.eTypeBulkVoice:
                {
                }
                break;

                case eTypeBulk.eTypeBulkAudio:
                {
                    int nLength = Marshal.SizeOf(typeof(slmSSpeechData)) * pSlmBulkRec.m_dataCounts.m_nNumAudio;
                    bulkRecords_DLL.m_pAudioRecord = Marshal.AllocHGlobal(nLength);
                    Marshal.Copy(new byte[nLength], 0, bulkRecords_DLL.m_pAudioRecord, nLength);
                    bReturn = SI_GetBulkDataEx_DLL(settings.m_nSamplePeriodInMSecs, ref bulkRecords_DLL);
                    if (bReturn)
                    {
                        IntPtr pPtr = bulkRecords_DLL.m_pAudioRecord;
                        for (int i = 0; i < pSlmBulkRec.m_dataCounts.m_nNumAudio; i++)
                        {
                            pSlmBulkRec.m_pAudioRecord[i] = (slmSSpeechData)Marshal.PtrToStructure(pPtr, typeof(slmSSpeechData));

                            pPtr = (IntPtr)((int)pPtr + Marshal.SizeOf(typeof(slmSSpeechData)));
                        }
                    }

                    Marshal.FreeHGlobal(bulkRecords_DLL.m_pAudioRecord);
                }
                break;
            }
